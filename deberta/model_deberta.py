...
class DebertaFeed(DebertaPreTrainedModel):
...
def init_deberta(param):
    
    config = DebertaConfig.from_pretrained(param['model_name'],
                                           num_labels=param['num_labels'])
    model = DebertaFeed.from_pretrained(param['model_name'], config=config, param=param)
    model.cuda()
...
def load_deberta(param):
    """use this when loading model for kaggle inference"""
    
    config = DebertaConfig.from_pretrained(param['kaggle_path'])
    model = DebertaFeed.from_pretrained(param['kaggle_path'], config=config, param=param)
    model.cuda()
